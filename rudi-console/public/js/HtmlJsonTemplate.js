    ...
    function buildElementChild(template){
        ...
        if (template.init) {
            for (let [func_name, args] of Object.entries(template.init))
                element[func_name].apply(element, args);
        }
...
function build(template, fragmentSet=undefined) {
    ...
    if (template instanceof Array) {
        ...
        for (let i = 0; i < template.length; i++) {
            frag.appendChild(buildElement(template[i], controller, fragmentSet));
        }

        return [frag, controller];
    } else {
        let element = buildElement(template, controller, fragmentSet);
