...
export let ListMixin = superclass => class extends superclass {
    constructor(...args) {
        super(...args);
        ...
        this.addEventListener('keydown', (e) => {
            switch (e.key) {
                case "Escape"   : this.hideList();      e.preventDefault(); break;
    ...
    */
    ...
    updateListPosition() {
        ...
        if (wrapperBr.bottom + optBr.height > window.innerHeight) { // Bottom overflow
            if (wrapperBr.top - optBr.height < 0) { // Top overflow
    ...
    toggleList() {
        ...
        if(shown) this.hideList(); else this.showList();
        return !shown
    }
...
export let SelectListMixin = superclass => class extends ListMixin(superclass) {
...
export let ActionMixin = superclass => class extends superclass {
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        switch (name) {
            case 'disabled' : this.action.toggleAttribute('disabled', newValue != null);   break;
            case 'readonly' : this.action.toggleAttribute('readonly', newValue != null);   break;
        }
...
export class ActionIcon extends HTMLElement {
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        switch (name) {
            case 'disabled' : this.#updateFocusable(newValue != null);          break;
            case 'readonly' : this.#updateFocusable(newValue != null);         break;
            case 'tabindex' : this.#updateFocusable(newValue != null); break;
        }
    }
...
export class BaseTextInput extends BaseInput {
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        switch (name) {
            case 'disabled': this.input.toggleAttribute("disabled", newValue != null); break;
            case 'readonly': this.input.toggleAttribute("readonly", newValue != null); break;
            case 'size':
...
export class SelectInput extends SelectInputs {
    ...
    setOptions(newOptions, noEmpty) {
        if (!noEmpty) { // Add an empty option
...
export class SelectMultipleInput extends SelectInputs {
    ...
    set value(newValue) {
        ...
        super.value = Array.from(this.#value).map((option) => option.name).join(', ');
    }
    ...
    select(option) {
        ...
        let val = Array.from(this.#value).map((option) => option.name).join(', ');
...
export class ActionCard extends HTMLElement {
    ...
    addAction(iconName, action, pos) {
        ...
        actionIcon.addEventListener('click', () => { action(this); });
...
export class BaseCardsBlock extends BaseInput {
    ...
    createCard(value) {
        ...
        card.addAction('close', (card) => { this.removeCard(card); }); // TODO handle focus when removing card
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        switch (name) {
            case 'disabled':
                for (let card of this.cardsIterator())
                    card.toggleAttribute('disabled', newValue != null);
                break;
            case 'readonly':
                for (let card of this.cardsIterator())
                    card.toggleAttribute('readonly', newValue != null);
                break;
            default: super.attributeChangedCallback(name, oldValue, newValue);
        }
    }
...
export class MultiTextArea extends ActionMixin(BaseInput) {
    constructor(...styles) {
        ...
        tabBar.addEventListener('keydown', (event) => {
            switch (event.key) {
                case "ArrowRight": this.tabNext();     event.preventDefault(); break;
                case "ArrowLeft" : this.tabPrevious(); event.preventDefault(); break;
            }
        });

        tabBar.addEventListener('keyup', (event) => {
            switch (event.key) {
                case 'Enter': this.textarea.focus(); break;
                case 'Backspace': this.removeTab(this.currentTab); break;
            }
        });
    }
    ...
    tabNext()     { return this.tabTo(this.currentTab?.nextElementSibling)     }
    tabPrevious() { return this.tabTo(this.currentTab?.previousElementSibling) }
...
export class FileCard extends ActionCard {
    ...
    set value(file) {
        ...
        else if (!(file instanceof File)) throw new SetValueError(this, file, new TypeError("Value should be a File or ForeignFile instance"));
        this.#value = file;
...
export class FilesInput extends ActionMixin(BaseCardsBlock) {
    constructor(...styles) {
        ...
        draggingZone.addEventListener('dragover', (ev) => { ev.preventDefault(); });
        let dragReset = () => { this.content.toggleAttribute('dropping', false); }
...
export class MapInput extends BaseInput {
    constructor(...styles) {
        ...
        var customControl =  L.Control.extend({
            ...
            onAdd: () => {
                ...
                this.fullScreenBtn.addEventListener('click', () => { this.toggleAttribute('fullscreen'); });
        ...
        // Basic controls
        this.drawControlFull = new L.Control.Draw({ draw: { polyline: false, circle: false, marker: false, circlemarker: false } });
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        switch (name) {
            case 'disabled': this.#updateControls(); break
            case 'readonly': this.#updateControls(); break
